...
exports.create = async (req, res) => {
  ...
  let correctWord = new CorrectWordAdvanced({
    question: question,
  ...
  const docCount = await TestCorrectWordAdvanced.countDocuments()
    .then((count) => {
  ...
  const data = await TestCorrectWordAdvanced.findOne({}).sort({ _id: -1 });
  ...
  const addToTest1 = async (data) => {
    ...
    if (questionNoLength !== 0 && data.questionNo.length < 4) {
      ...
      const modelLength = TestCorrectWordAdvanced.length;
      ...
      const addToTest = new TestCorrectWordAdvanced(newQuizData);
...
exports.getTestNo = async (req, res) => {
  try {
    ...
    const documents = await TestCorrectWordAdvanced.findOne({}).skip(
      testNo
    );
    if (documents === null) {
